    ...
    <div class="fullscreen checkOut">
        ...
        <div class="content">
            <div class="left">
                ...
                <div class="leftContent">
                    <ul class="orderList">
                        <li v-for="(order,index) in orderList" :key="index">
                            ...
                            <span v-if="order.Price==order.ActualPrice">￥{{(order.Price*order.Number).toFixed(2)}}</span>
                            <span v-else><span class="old">￥{{(order.Price*order.Number).toFixed(2)}}</span>￥{{(order.ActualPrice*order.Number).toFixed(2)}}</span>
                        </li>
                    </ul>
                ...
                <div class="total">
                    <div>总计：<amount :value="priceAmount"/></div>
                    <div>卡项消耗：<amount :value="cardAmount"/></div>
                    <div>应付金额：<amount :value="postData.ActualAmount"/></div>
                </div>
            ...
            <div class="billCont">
                ...
                <div class="projectWrap">
                    <ul v-for="(order,index) in postArr" :key="index" class="projectList">
                        ...
                        <li class="name">
                            <span >{{order.ItemName}}</span>
                            <span><span class="old">￥{{order.Price.toFixed(2)}}</span>￥{{order.ActualPrice.toFixed(2)}}</span>
                            <!-- <span> ￥{{order.ActualPrice.toFixed(2)}}</span> -->
                        </li>
    ...
    methods: {
            ...
            this.$axios.post(url, postData).then(res => {
                if (res.data.Code === '0') {
                    ...
                    if(data.Status){
                        ...
                        this.handleAmountChange(this.postData.Amount,this.priceAmount - this.cardAmount);
                    }
        ...
        getNewArr(arr){
            ...
            for(var i = 0; i < newArr.length; i++) {
                let flag = newArr2.find(item => item.ItemId==newArr[i].ItemId && item.ActualPrice==newArr[i].ActualPrice && item.ItemType==newArr[i].ItemType)
                if(!flag){
            ...
            for(let i=0;i<newArr.length;i++){
                for(let j=0;j<newArr2.length;j++){
                    if(newArr[i].ItemId == newArr2[j].ItemId && newArr[i].ActualPrice == newArr2[j].ActualPrice && newArr[i].ItemType == newArr2[j].ItemType){
                        newArr2[j].Number++;
                    }
                }
            }
            ...
            for(let j=0;j<this.orderList.length;j++){
                this.priceAmount += this.orderList[j].Price * this.orderList[j].Number;
                if(this.orderList[j].CardProjectId){
                    this.cardAmount += this.orderList[j].Price * this.orderList[j].Number;
                }
            }
            // console.log(this.cardAmount,this.priceAmount)
            // console.log('mmm',newArr2)
        },
        //会员信息
        getInfo() {
        ...
        withoutDiscount() {
            ...
            this.handleAmountChange(this.postData.Amount,this.priceAmount - this.cardAmount,false);
        },
        handleConfirmDiscount() {
            this.showDiscount = false;
            // this.handleAmountChange(this.postData.Amount, true)
            this.handleAmountChange(this.postData.Amount,this.priceAmount - this.cardAmount,true);
        },
        handleAmountChange(e, notCardAmount,discount = true) {
            let actual = e;
            this.postData.Amount = e;
            //折扣，如8折 已经使用折扣卡项不可打折,买卡不可打折
            if (this.postData.Discount) {
                this.postData.DiscountAmount = (notCardAmount * (10 - this.postData.Discount) / 10).toFixed(2)
