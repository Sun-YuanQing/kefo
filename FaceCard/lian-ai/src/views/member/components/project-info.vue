<template>
    <div class="left">
        <RadioGroup @on-change="onRadioChange" v-model="radio" type="button">
            ...
            <Radio :label="3" v-if="!isTraveler">卡项</Radio>
        </RadioGroup>
        ...
        <div class="empty" v-if="isLoaded3 && tabContent.length==0 && radio == 3">还没有卡项，点击<router-link to="/cardManage">添加卡项</router-link>吧！</div>
        <template v-else>
            <transition-group name="list-transition" tag="ul" class="itemList" v-if="radio==1 || radio==2">
            ...
            <transition-group name="list-transition" tag="ul" class="itemList" v-else>
                <li v-for="item in tabContent" :key="item.CardId" @click="addProject(item)" class="list-transition-item">
                    <p>{{item.CardName}}</p>
                    <p>￥{{item.CardPrice.toFixed(2)}}</p>
                </li>
            </transition-group>
        </template>
        ...
        <Modal v-model="isAddProject" :title="projectTitle" class="addModal"
            ...
            <div class="addModalConten">
                <ul class="workList" >
                    ...
                    <li class="empty" style="text-align: left;color: #999;"  
                        v-if="(radio == '2' || radio == '3') && saleWays.length==0">点击
...
export default {
    ...
    computed:{
        tabList: function(){
            if(this.radio == 1){
                ...
                return [{Name: '次卡',Id: '次卡'},{Name: '折扣卡',Id: '折扣卡'},{Name: '体验卡',Id: '体验卡'}]
            }
        ...
        tabContent: function(){
            if(this.radio == 1){
                ...
                if(this.tabValue==0){
                    ...
                    return this.cardList.filter(item => item.ClassType == this.tabValue);
                }
            }
    ...
    methods: {
        onRadioChange(e){
            this.tabValue = 0;
            if(this.radio == 1){
                this.workList =  this.professions;
            }else if(this.radio == 2){
                this.workList = this.saleWays;
                this.getProductTypes(); //产品类别
                this.getProducts();    //产品列表
            }else{
                this.workList = this.saleWays;
        ...
        addOrder(){
            ...
            let serviceStaff = Object.values(this.selectedEmployee);
            let detail = this.orderDetail;
            // console.log('detail',detail)
            if(Object.keys(this.selectedEmployee).length > 0){
                if(this.orderDetail.ItemType=="3"){
                    //卡项详情
                    let url = '/CardProject/GetCardPeojectByCardId?cardId=' + this.orderDetail.ItemId;
                    // this.$Spin.show();
                    ...
                    this.$axios.post(url, {UserId: this.userId,ItemId: this.orderDetail.ItemId}).then(res => {
                        if (res.data.Code === '0') {
                            this.orderDetail = {
                                ...detail,
                                ServiceStaff: serviceStaff,
                                cardCount: res.data.Data.Count,
                            }
                            this.orderList.push(this.orderDetail)
                            this.$emit('getOrder',this.orderList)
                        } else {
        ...
        addProject(item){
            ...
            // 卡项
            if(this.radio=='3'){
                this.projectTitle = item.CardName;
                this.orderDetail = {
                        ItemName: item.CardName,
                        ItemId: item.CardId,
                        ItemType: '3',
                        Price: item.CardPrice,
                        ActualPrice: item.CardPrice
                }
            }else{
        ...
        getCardList(){
            ...
            this.$Spin.show();
            this.$axios.post(url,{ClassType: '',Key: ''}).then(res => {
                this.$Spin.hide();
                const { Code, Message, Data } = res.data;
                if (Code === '0') {
                    this.isLoaded3 = true;
                    this.cardList = Data.filter(item => item.State == '1');
                } else {
                    this.$Message.error(Message);
                }
            }).catch(e => {
