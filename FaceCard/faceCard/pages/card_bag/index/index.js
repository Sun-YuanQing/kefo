...
Page({
  ...
  toggle: function() {
    ...
    this.setData({
      showBigCard: !this.data.showBigCard,
      cardList: [],
      totalCards: 0
    })
    this.queryCard();
  ...
  queryCard: function() {
    ...
    server.sendRequest({
      url: '/CardProject/GetUserCenter',
      data: {
        'phone': wx.getStorageSync("PhoneNumber"), //  //app.globalData.phoneNumber
        IsInvalid: self.data.inval
      },
      method: "POST",
      success(res) {
        wx.hideLoading();
  ...
  navigate: function(e) {
    ...
    let card = options.VipList[0];
    if (options.VipCount == 1) {
      wx.navigateTo({
