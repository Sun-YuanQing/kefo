<template>
  <div class="productClass">
    <Header title="产品类别"></Header>
    <div class="whiteBack">
      <div class="neiBack">
        <div class="title">产品类别</div>
        <div class="subhead">点击“+”可新增产品类别，点击已添加的产品类别可再次编辑，点击右上角的“x”可删除产品类别。</div>
        <div class="line_all">
          <div
...
export default {
  ...
  methods: {
    ...
    isOk() {
      ...
      that.axios.post('/Lable/EditProductType', allData)
        .then(res => {
          if (res.data.Code == 0) {
            if (that.showData.Id) {
    ...
    toDel(e) {
      this.$Modal.confirm({
        title: '请确认是否删除',
        onOk: () => {
          this.$axios.post('/Lable/DelProductTypeById', { Id: e })
            .then(res => {
              if (res.data.Code === '0') {
