<template>
    <div class="seckillDetail">
        <Header title="秒杀详情" @change-router="$router.back()"></Header>
        <div class="contentWrap">
            <div class="basicInfo">
                ...
                <Form :label-width="110" inline>
                    ...
                    <FormItem label="活动状态：">
                        <div>{{formData.SeckState == 1 ? '进行中' : formData.SeckState == 0 ? '未开始' : '已结束'}}</div>
                    </FormItem>
                    <FormItem label="活动限购：">
            ...
            <div class="bottom">
                <Button type="primary" @click="$router.push({path: '/editSeckill',query: {id: id}})">编辑</Button>
            </div>
        </div>
    </div>
</template>
...
import { getSeckillDetail } from '@/api/marketing.js'
export default {
    components: {Header},
    data(){
        return{
