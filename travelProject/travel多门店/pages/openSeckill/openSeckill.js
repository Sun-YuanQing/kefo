...
Page({
  ...
  getKillPrice(e) {
    this.data.formVal.KillPrice = e.detail.value;
    this.setData({
      formVal: this.data.formVal
    })
  },
  ...
  getStockNum(e) {
    this.data.formVal.StockNum = e.detail.value == 0 ? 1 : e.detail.value;
    this.setData({
      formVal: this.data.formVal
    })
  },
  ...
  reviseData(e) {
    if ((this.data.goodsDetail.Type == '1' || this.data.goodsDetail.Type == '2') && !this.data.formVal.DepartureDate) {
      $Message({
        content: '出发日期不为空！',
        type: 'error'
      });
